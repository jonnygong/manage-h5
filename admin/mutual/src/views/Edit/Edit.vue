...
<script>
  ...
  export default {
    ...
    computed: {
      previewSrc() {
        return `http://api.mp.kfw001.com/${this.templateUrl[this.formData.activity.template]}/Activity/index?code=${this.formData.code}`
      },
      whiteSrc() {
        return `http://api.mp.kfw001.com/${this.templateUrl[this.formData.activity.template]}/Activity/white?code=${this.formData.code}`
      }
    },
    methods: {
      // 添加奖品按钮
